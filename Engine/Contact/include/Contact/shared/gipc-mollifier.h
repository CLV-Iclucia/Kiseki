//
// gipc-mollifier.h 鈥?shared (C++ + HLSL) GIPC edge-edge mollifier.
//
...
// contact::gipc::computeMollifiedBarrier exactly.
//
// Key simplification (verified against the autogen pFpx_pee): the mollifier
// PFPx (9x12) has only TWO non-zero rows 鈥?row 4 (the tangential / I1 column,
// "r4") and row 8 (the distance / I2 column, "r8"). r8 is bit-identical to the
// plain edge-edge contact vector shGipcPFPx_EE().v (same d/d虃 column), so it is
...
#include <Contact/shared/gipc-pfpx.h>     // shGipcPFPx_EE (r8) + shDistanceSqrLineLine
#include <Contact/shared/gipc-barrier.h>  // shared barrier conventions

SH_NS_BEGIN

// 蔚_x = 1e-3 路 |ea_rest|虏 路 |eb_rest|虏  (rest-pose mollifier threshold).
...
// m_eps(c) = 2c/蔚 鈭?c虏/蔚虏  (c < 蔚), else 1.   c = I1 = ||ea脳eb||虏.
...
// (current cross虏 < rest-pose 蔚_x). Caller routes such EE pairs through the
...
struct ShMollifiedEE {
    real grad[12];     // x = [ea0,ea1,eb0,eb1]
    real w[2][12];     // rank-1 Hessian generators: H = 危 lam[k]路w[k]路w[k]岬€
...
SH_INLINE ShMollifiedEE shMollifiedBarrierEEWithOffset(
    real3 ea0, real3 ea1, real3 eb0, real3 eb1,
    real3 rea0, real3 rea1, real3 reb0, real3 reb1,
    real dHat, real kappa, real surfaceOffset) {
    ShMollifiedEE res;
    ...
    // r8 = the distance column = plain EE contact vector (same d/d虃 column).
    ShGipcPfpx ee = shGipcPFPx_EE(ea0, ea1, eb0, eb1, dHat);
    if (!ee.valid) return res;                 // degenerate -> inactive
    real dSqr = shDistanceSqrLineLine(ea0, ea1, eb0, eb1);
    real gap = shMax(shSqrt(dSqr) - surfaceOffset, dHat * 1.0e-12);
    real I2 = (gap * gap) / dHatSqr;         // = gap^2 /dHat ^2
...
SH_INLINE ShMollifiedEE shMollifiedBarrierEE(
    ...
    return shMollifiedBarrierEEWithOffset(
        ea0, ea1, eb0, eb1, rea0, rea1, reb0, reb1, dHat, kappa, 0.0);
}

// Mollified edge-edge barrier energy (for line search / energy parity).
//   E = kappa 路 m(I1) 路 艥虏 路 (1-I2)虏 路 ln虏(I2)
SH_INLINE real shMollifiedEnergyEEWithOffset(
    real3 ea0, real3 ea1, real3 eb0, real3 eb1,
    real3 rea0, real3 rea1, real3 reb0, real3 reb1,
    real dHat, real kappa, real surfaceOffset) {
    real dHatSqr = dHat * dHat;
...
SH_INLINE real shMollifiedEnergyEE(
    ...
    real3 rea0, real3 rea1, real3 reb0, real3 reb1,
    real dHat, real kappa) {
    return shMollifiedEnergyEEWithOffset(
        ea0, ea1, eb0, eb1, rea0, rea1, reb0, reb1, dHat, kappa, 0.0);
}
