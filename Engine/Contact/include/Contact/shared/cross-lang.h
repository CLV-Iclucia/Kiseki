//
// cross-lang.h 鈥?C++/HLSL cross-language compatibility shim.
//
...
//   SH_NS_BEGIN/END  wrap shared symbols in `ksk::engine::contact::shared` on the CPU,
...
#define KISEKI_FEM_INCLUDE_FEM_SHARED_CROSS_LANG_H_
  ...
  namespace ksk::engine::contact::shared {
  using real  = double;
  using real3 = glm::dvec3;
  }  // namespace ksk::engine::contact::shared

  #define SH_NS_BEGIN  namespace ksk::engine::contact::shared {
  #define SH_NS_END    }
...
SH_INLINE real shMax(real a, real b) {
    return a > b ? a : b;
}
