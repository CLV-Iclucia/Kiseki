...
SH_INLINE sh_real shMollifierEpsX(sh_real3 rea0, sh_real3 rea1,
                                  sh_real3 reb0, sh_real3 reb1) {
    sh_real3 ra = rea1 - rea0;
    sh_real3 rb = reb1 - reb0;
    return 1e-3 * shDot(ra, ra) * shDot(rb, rb);
}

// m_eps(c) = 2c/ε − c²/ε²  (c < ε), else 1.   c = I1 = ||ea×eb||².
SH_INLINE sh_real shMollifierValue(sh_real c, sh_real epsX) {
...
SH_INLINE bool shEEUsesMollifier(sh_real3 ea0, sh_real3 ea1,
                                 sh_real3 eb0, sh_real3 eb1,
                                 sh_real3 rea0, sh_real3 rea1,
                                 sh_real3 reb0, sh_real3 reb1) {
    sh_real3 cr = shCross(ea1 - ea0, eb1 - eb0);
    sh_real I1 = shDot(cr, cr);
    return I1 < shMollifierEpsX(rea0, rea1, reb0, reb1);
}

// ---- mollifier scalar coefficients (no kappa; transcribed from Barrier) ----

SH_INLINE sh_real shMollifierP1(sh_real I1, sh_real I2, sh_real epsX, sh_real sHat2) {
...
SH_INLINE sh_real shMollifierP2(sh_real I1, sh_real I2, sh_real epsX, sh_real sHat2) {
...
SH_INLINE sh_real shMollifierLambda10(sh_real I1, sh_real I2, sh_real epsX, sh_real sHat2) {
    if (I2 >= 1.0) return 0.0;
    sh_real L2 = shLog(I2);
...
SH_INLINE sh_real shMollifierLambda20(sh_real I1, sh_real I2, sh_real epsX, sh_real sHat2) {
    if (I2 >= 1.0 || I2 <= 0.0) return 0.0;
    sh_real L2 = shLog(I2);
...
SH_INLINE sh_real shMollifierLambdaG1G(sh_real I1, sh_real I2, sh_real epsX, sh_real sHat2) {
    if (I2 >= 1.0 || I2 <= 0.0 || I1 <= 0.0) return 0.0;
    sh_real L2 = shLog(I2);
    sh_real c   = shSqrt(I1);
    sh_real F33 = shSqrt(I2);
...
struct ShPD2 {
    sh_real val[2];
    sh_real vec[2][2];   // column k: (vec[k][0], vec[k][1])
    int     num;
};

SH_INLINE ShPD2 shMakePD2x2(sh_real a, sh_real b, sh_real d) {
    ShPD2 r;
    r.num = 0;
    ...
    sh_real trace = a + d;
    sh_real det   = a * d - b * b;
    sh_real disc  = trace * trace - 4.0 * det;
    disc = shMax(disc, 0.0);
    sh_real sd = shSqrt(disc);
    sh_real lam[2];
    lam[0] = 0.5 * (trace + sd);
    lam[1] = 0.5 * (trace - sd);
    ...
    for (int k = 0; k < 2; ++k) {
        ...
        sh_real r0 = a - lam[k];
        sh_real r1 = b;
        sh_real mag = (r0 < 0.0 ? -r0 : r0) + (r1 < 0.0 ? -r1 : r1);
        if (mag < 1e-30) { r0 = -b; r1 = d - lam[k]; }
        // eigenvector = normalize((-r1, r0))
        sh_real v0 = -r1, v1 = r0;
        sh_real nrm = shSqrt(v0 * v0 + v1 * v1);
...
struct ShMollifiedEE {
    sh_real grad[12];     // ∂E/∂x, x = [ea0,ea1,eb0,eb1]
    sh_real w[2][12];     // rank-1 Hessian generators: H = Σ lam[k]·w[k]·w[k]ᵀ
    sh_real lam[2];       // eigenvalues (>= 0), already including kappa
...
SH_INLINE ShMollifiedEE shMollifiedBarrierEE(
    ...
    sh_real3 rea0, sh_real3 rea1, sh_real3 reb0, sh_real3 reb1,
    sh_real dHat, sh_real kappa) {
    ShMollifiedEE res;
    ...
    sh_real epsX = shMollifierEpsX(rea0, rea1, reb0, reb1);
    ...
    sh_real invSqrtI1 = 1.0 / shSqrt(I1);
    sh_real t81 = (t13 * t45 + t14 * t46) * invSqrtI1;
    sh_real t82 = (t12 * t46 + t13 * t47) * invSqrtI1;
    sh_real t83 = (t19 * t45 + t20 * t46) * invSqrtI1;
    sh_real t84 = (t18 * t46 + t19 * t47) * invSqrtI1;
    sh_real t85 = (t12 * t45 - t14 * t47) * invSqrtI1;
    sh_real t86 = (t18 * t45 - t20 * t47) * invSqrtI1;
    sh_real r4[12];
    ...
    sh_real c   = shSqrt(I1);
    sh_real F33 = shSqrt(I2);
    sh_real p1 = kappa * shMollifierP1(I1, I2, epsX, sHat2);
    sh_real p2 = kappa * shMollifierP2(I1, I2, epsX, sHat2);
    ...
    sh_real l10 = kappa * shMollifierLambda10(I1, I2, epsX, sHat2);
    sh_real l20 = kappa * shMollifierLambda20(I1, I2, epsX, sHat2);
    sh_real lg  = kappa * shMollifierLambdaG1G(I1, I2, epsX, sHat2);
    ShPD2 pd = shMakePD2x2(l10, lg, l20);
    res.rank = pd.num;
    for (int k = 0; k < pd.num; ++k) {
        sh_real a = pd.vec[k][0], b = pd.vec[k][1];
        res.lam[k] = pd.val[k];
        for (int i = 0; i < 12; ++i)
...
SH_INLINE sh_real shMollifiedEnergyEE(
    sh_real3 ea0, sh_real3 ea1, sh_real3 eb0, sh_real3 eb1,
    sh_real3 rea0, sh_real3 rea1, sh_real3 reb0, sh_real3 reb1,
    sh_real dHat, sh_real kappa) {
    sh_real dHatSqr = dHat * dHat;
    ...
    sh_real dSqr = shDistanceSqrLineLine(ea0, ea1, eb0, eb1);
    sh_real I2 = dSqr / dHatSqr;
    if (I2 >= 1.0 || I2 <= 0.0) return 0.0;
    sh_real epsX = shMollifierEpsX(rea0, rea1, reb0, reb1);
    sh_real m  = shMollifierValue(I1, epsX);
    sh_real L2 = shLog(I2);
    sh_real t2 = 1.0 - I2;
    sh_real sHat2 = dHatSqr * dHatSqr;
