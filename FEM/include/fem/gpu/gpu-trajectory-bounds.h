...
class TrajBoundCS final : public sim::rhi::ComputeShader<TrajBoundCS> {
public:
    DECLARE_COMPUTE_SHADER(TrajBoundCS);

    SHADER_PARAMS_BEGIN(Params)
        SHADER_PARAM_UAV   (sim::rhi::BufferRef, outLo);
        SHADER_PARAM_UAV   (sim::rhi::BufferRef, outHi);
...
public:
    explicit GpuTrajectoryBounds(sim::rhi::Device& device);
    ...
    void record(sim::rhi::CommandList& cmd,
                const sim::rhi::BufferRef& x,
                const sim::rhi::BufferRef& p,
                const sim::rhi::BufferRef& conn,
                const sim::rhi::BufferRef& alphaBuf,
                const sim::rhi::BufferRef& outLo,
                const sim::rhi::BufferRef& outHi,
                uint32_t numPrims, uint32_t vertsPerPrim);

    // Convenience: self-submitting version.
    void compute(const sim::rhi::BufferRef& x,
                 const sim::rhi::BufferRef& p,
                 const sim::rhi::BufferRef& conn,
                 const sim::rhi::BufferRef& alphaBuf,
                 const sim::rhi::BufferRef& outLo,
